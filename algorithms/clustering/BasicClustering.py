...
class BasicClustering(ClusteringUpdateWrapper):
    ...
    def _write_results(self) -> None:
        """
        Write a Data frame containing the barcodes and the clustering labels to a file.
        """
        try:
            pd.DataFrame(self.data.obs["current_clustering"]).to_csv(
                op.join(self.data.uns["clustering_dir"], "cluster_labels.tsv"),
                sep="\t",
                header=True,
                index=True,
            )
