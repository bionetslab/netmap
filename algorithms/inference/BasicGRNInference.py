    ...
    def _check_GRN_convergence(self, consistency):
        ...
        if i>=1:
            ...
            for GRN_old, GRN_new in zip(self.data.uns['GRNs'][f'iteration_{im1!r}']):
                ...
                if int(np.sum(self.data.varp[self.data.uns['GRNs'][f'iteration_{im1!r}'][GRN_old]])) * consistency >= number_of_consistent_edges and int(np.sum(self.data.varp[self.data.uns['GRNs'][f'iteration_{i!r}'][GRN_new]])) * consistency >= number_of_consistent_edges:
